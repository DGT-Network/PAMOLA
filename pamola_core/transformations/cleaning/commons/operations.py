...
import numpy as np
import pandas as pd
from pamola_core.transformations.cleaning.commons.base import (
    FieldOperation as BaseFieldOperation,
)

# Import metrics collector
from pamola_core.transformations.cleaning.commons.metrics import (
    create_metrics_collector,
)
from pamola_core.transformations.cleaning.commons.metrics_report import (
...
class FieldOperation(BaseFieldOperation, BaseOperation):
    ...
    def execute(self, data_source: Any, task_dir: Path, reporter: Any, **kwargs) -> OperationResult:
        ...
        **kwargs : dict
        ...
        try:
            ...
            result = self._generate_output(df, task_dir, duration, operation_params)
    ...
    def _generate_output(self, df: pd.DataFrame, task_dir: Path, 
                        duration: float, operation_params: dict) -> OperationResult:
        """
        ...
        metrics = self._generate_visualizations(metrics, visualizations_dir)
        
        # Save results
        output_path = self._save_result(df, output_dir)
        metrics_path = self._save_metrics(metrics, output_dir)
    ...
    def _generate_visualizations(self, metrics: dict, output_dir: Path) -> dict:
        """
    ...
    def _add_artifacts_to_result(self, result: OperationResult, 
        ...
        if output_path:
            result.add_artifact(
                "csv", output_path, f"Processed data for {self.field_name}"
            )
        
        result.add_artifact(
            "json", metrics_path, f"Metrics for {self.name} on {self.field_name}", "metric"
        )
