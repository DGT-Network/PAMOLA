...
class CleanDataError(Exception):
    """Base exception class for clean_data module errors."""
    pass


class ValidationError(CleanDataError):
    """Exception raised for validation errors."""
    pass


class ResourceError(CleanDataError):
    """Exception raised for resource-related errors (memory, disk, etc.)."""
    pass


class MappingError(CleanDataError):
...
class BaseGenerator(abc.ABC):
    """
    Base abstract class for all clean data generators.
    ...
    def generate(self, count: int, **params) -> List[Any]:
        """
        Generates the specified number of clean values.
    ...
    def generate_like(self, original_value: Any, **params) -> Any:
        """
        Generates a clean value similar to the original.
...
class BaseMapper(abc.ABC):
    ...
    def map(self, original_value: Any, **params) -> Any:
        """
        Maps an original value to a clean one.
    ...
    def restore(self, synthetic_value: Any) -> Optional[Any]:
        """
        Attempts to restore the original value from a clean one.

        Parameters:
        -----------
        synthetic_value : Any
            clean value to restore from
...
class FieldOperation(BaseOperation):
    ...
    def __init__(self, field_name: str, mode: str = "REPLACE", output_field_name: Optional[str] = None):
        ...
        if self.mode == "ENRICH" and not self.output_field_name:
            self.output_field_name = f"{field_name}_clean"
