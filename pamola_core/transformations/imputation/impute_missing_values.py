...
from pamola_core.transformations.schemas.impute_missing_values_op_core_schema import ImputeMissingValuesConfig


@register(version="1.0.0")
class ImputeMissingValuesOperation(TransformationOperation):
    """Operation for imputing missing or invalid values in datasets."""
    ...
    def __init__(
        ...
        **kwargs,
        ...
        field_strategies : dict, optional
            Imputation strategies per field
                ...
                if is_numeric_dtype(batch[output_field_name]) or data_type == "numeric":
                    ...
                    if imputation_strategy == "mean":
                        ...
                        batch[output_field_name] = (
                            batch[output_field_name]
                            .astype(float)
                            .fillna(batch[output_field_name].mean())
                        )
                    ...
                    if imputation_strategy == "median":
                        ...
                        batch[output_field_name] = batch[output_field_name].fillna(
                            batch[output_field_name].median()
                        )
                ...
                if (
                    ...
                    if imputation_strategy == "constant":
                        if constant_value and constant_value not in invalid_values:
                            if (
                                constant_value
                                not in batch[output_field_name].cat.categories
                            ):
                                batch[output_field_name] = batch[
                                    output_field_name
                                ].cat.add_categories([constant_value])
                    ...
                    if imputation_strategy == "random_sample":
                        ...
                        batch.loc[
                            batch[output_field_name].isna(), output_field_name
                        ] = np.random.choice(choice_values, size=num_missing)
                        batch[output_field_name] = batch[output_field_name].astype(
                            "category"
                        )
    ...
    def _generate_visualizations(
        ...
        **kwargs,
        ...
        try:
            ...
            viz_data = {
                field: statics["count"]
                for field, statics in metrics["imputed_values"].items()
            }
            viz_path = (
                viz_dir
                / f"{self.operation_name.lower()}_imputed_values_count_{operation_timestamp}.png"
            )
            ...
            for processed_field in fields_modified + fields_added:
                ...
                if is_numeric_dtype(original_df[original_field]):
                    ...
                    if viz_result.startswith("Error"):
                        ...
                        visualization_paths[f"histograms_original_{original_field}"] = (
                            viz_path
                        )
                ...
                if is_numeric_dtype(original_df[original_field]) and is_numeric_dtype(
                    ...
                    viz_path = (
                        viz_dir
                        / f"{self.operation_name.lower()}_boxplot_{original_field}_{operation_timestamp}.png"
                    )
                    ...
                    if is_numeric_dtype(original_df[original_field]):
                        ...
                        viz_path = (
                            viz_dir
                            / f"{self.operation_name.lower()}_boxplot_original_{original_field}_{operation_timestamp}.png"
                        )
                        viz_result = create_boxplot(
                            data=viz_data,
                            output_path=viz_path,
                        ...
                        if viz_result.startswith("Error"):
                            ...
                            visualization_paths[
                                f"boxplot_original_{original_field}"
                            ] = viz_path
                    ...
                    if is_numeric_dtype(processed_df[processed_field]):
                        ...
                        viz_path = (
                            viz_dir
                            / f"{self.operation_name.lower()}_boxplot_processed_{processed_field}_{operation_timestamp}.png"
                        )
                        viz_result = create_boxplot(
                            data=viz_data,
                            output_path=viz_path,
                        ...
                        if viz_result.startswith("Error"):
                            ...
                            visualization_paths[
                                f"boxplot_processed_{processed_field}"
                            ] = viz_path
...
def create_impute_missing_values_operation(**kwargs) -> ImputeMissingValuesOperation:
    """
