...
- Null pattern:
    ...
    """
        if orig
        else "### Null Analysis\nNo null analysis for original data available.\n"
    )
...
def _format_original_data(metrics: Dict[str, Any]) -> str:
    ...
    # Prefer non-empty length_stats first, then numeric_stats
    stats = {}
    if orig.get("length_stats"):
        stats = orig["length_stats"]
    elif orig.get("numeric_stats"):
        stats = orig["numeric_stats"]

    null_pattern = orig.get("null_pattern", {})

    # Format null pattern
    null_pattern_str = "\n".join(
        [f"  - {k}: {v}" for k, v in null_pattern.items()]
    ) if null_pattern else "  - None"

    # Prepare stats safely
    if stats:
        length_info = (
        ...
        length_info = "No statistics available."
...
def _format_generated_data(metrics: Dict[str, Any]) -> str:
    ...
    # Prefer non-empty length_stats first, then numeric_stats
    stats = {}
    if gen.get("length_stats"):
        stats = gen["length_stats"]
    elif gen.get("numeric_stats"):
        stats = gen["numeric_stats"]
...
- Reversibility rate: {trans.get('reversibility_rate', 0):.2%}
    """
        if trans
        else "### Transformation\nNo transformation Metrics available.\n"
    )
