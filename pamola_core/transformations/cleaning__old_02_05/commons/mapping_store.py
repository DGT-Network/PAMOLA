"""
MappingStore for clean_data package.
...
logger = pamola_logging.get_logger("pamola_core.clean_data.mapping_store")
...
class MappingStore:
    ...
    def add_mappings_from_series(self, field_name: str, originals: pd.Series, synthetics: pd.Series) -> None:
        """
        Adds mappings for a field from original and synthetic Series,
        even when original or synthetic is NaN, but skips when both are NaN.

        Args:
            field_name (str): The field name.
            originals (pd.Series): Original values.
            synthetics (pd.Series): Synthetic values.
        """
        for original, synthetic in zip(originals, synthetics):
            # Skip if both are NaN
            if pd.isna(original) and pd.isna(synthetic):
                continue

            # Add mapping if values are different
            if original != synthetic:
                self.add_mapping(field_name, original, synthetic)
