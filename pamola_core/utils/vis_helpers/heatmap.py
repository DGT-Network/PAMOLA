"""
Heatmap implementations for the visualization system.

This module provides the implementations for heatmaps using both
Plotly and Matplotlib backends. Heatmaps are particularly useful
for visualizing matrix data such as correlation matrices.
"""
...
def prepare_data_for_heatmap(data: Union[Dict[str, Dict[str, float]], pd.DataFrame, np.ndarray]) -> Tuple[
...
def prepare_text_colors(matrix: np.ndarray,
                        annotation_color_threshold: Optional[float] = 0.5) -> np.ndarray:
    """
    ...
    if annotation_color_threshold is not None:
        ...
        normalized_data = (matrix - z_min) / (z_max - z_min) if z_max > z_min else np.ones_like(matrix)

        # Create a 2D array of colors
        text_colors = np.where(normalized_data > annotation_color_threshold, 'white', 'black')
        for i in range(text_colors.shape[0]):
            for j in range(text_colors.shape[1]):
                if np.isnan(matrix[i, j]):
                    text_colors[i, j] = 'rgba(0,0,0,0)'  # Make text transparent for NaN values
    else:
        # Use all black text
...
def prepare_text_values(matrix: np.ndarray, annotation_format: str = ".2f") -> np.ndarray:
    """
...
def handle_mask_values(matrix: np.ndarray, mask_values: Optional[np.ndarray] = None) -> np.ndarray:
    """
...
def create_matplotlib_imshow(ax, masked_matrix, cmap_with_alpha, kwargs):
    ...
    try:
        ...
        logger.warning(f"Invalid aspect value '{aspect_value}'. Using default instead.")
        im = ax.imshow(
            masked_matrix,
            cmap=cmap_with_alpha,
            vmin=vmin,
    ...
    def update(self, fig: 'go.Figure', **kwargs) -> 'go.Figure':
        ...
        **kwargs:
        ...
        if 'data' in kwargs:
            ...
            annotation_color_threshold = kwargs.get('annotation_color_threshold', 0.5)
...
class MatplotlibHeatmap(MatplotlibFigure):
    ...
    def create(self,
        ...
        **kwargs:
        ...
        im = create_matplotlib_imshow(ax, masked_matrix, cmap_with_alpha, kwargs)
        ...
        if annotate:
            ...
            threshold = annotation_color_threshold if annotation_color_threshold is not None else 0.5
            ...
            for i in range(len(y_labels)):
                for j in range(len(x_labels)):
                    ...
                    if annotation_color_threshold is not None:
                        norm_val = (val - z_min) / (z_max - z_min) if z_max > z_min else 0.5
                    else:
    ...
    def update(self, fig: 'plt.Figure', **kwargs) -> 'plt.Figure':
        ...
        **kwargs:
        ...
        if not isinstance(fig, plt.Figure):
            logger.warning("Cannot update non-Matplotlib figure with MatplotlibHeatmap")
            return fig
        ...
        if 'data' in kwargs:
            ...
            data = kwargs['data']
            title = kwargs.get('title', fig.texts[0].get_text() if fig.texts else '')
