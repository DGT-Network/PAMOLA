"""
Boxplot implementations for the visualization system.
...
class PlotlyBoxPlot(PlotlyFigure):
    ...
    @staticmethod
    def _prepare_data_for_boxplot(data: Union[Dict[str, List[float]], pd.DataFrame, pd.Series]) -> pd.DataFrame:
        """
    ...
    def _prepare_box_trace_args(
        ...
        try:
            ...
            if len(values_array) == 1:
                logger.warning(f"Column '{column}' has only one data point. Box plot may not display properly.")
            ...
            logger.error(f"Error preparing box trace arguments: {e}")
            raise
        ...
        try:
            ...
            for i, column in enumerate(processed_data.columns):
                try:
                    ...
                    color = kwargs.get(f'color_{column}',
                    ...
                    box_args = self._prepare_box_trace_args(
                        ...
                        orientation,
                        color,
                        {**kwargs, 'show_points': show_points, 'notched': notched,
                         'points': points, 'box_width': box_width}
                    )

                    # Add box trace to the figure
                    fig.add_trace(go.Box(**box_args))
                except Exception as e:
                    logger.warning(f"Could not create box trace for column '{column}': {e}")
                    continue
            ...
            if len(fig.data) == 0:
                return self.create_empty_figure(
                    title=title,
                    message="Could not create any valid box traces"
                )
        ...
        try:
            ...
            if 'data' in kwargs:
                try:
                    ...
                    existing_names = [getattr(trace, 'name', '') for trace in fig.data]

                    # Determine if we need to rebuild traces
                    rebuild_traces = (
                            set(existing_names) != set(processed_data.columns) or
                            len(existing_names) != len(processed_data.columns)
                    )
                        ...
                        for i, column in enumerate(processed_data.columns):
                            try:
                                if column in existing_names:
                                    ...
                                    # Update data safely
                                    if orientation == "v":
                                        fig.data[trace_idx]['y'] = processed_data[column].dropna().values
                                    else:
                                        fig.data[trace_idx]['x'] = processed_data[column].dropna().values
                            except Exception as e:
                                logger.warning(f"Could not update trace for column '{column}': {e}")
                                continue
                except Exception as e:
        ...
        **kwargs :
        ...
        try:
            ...
            for i, box in enumerate(bp['boxes']):
                box_color = kwargs.get(f'color_{df.columns[i]}',
                                       kwargs.get('color', colors[i % len(colors)]))
        ...
        **kwargs :
        ...
        try:
            ...
            if not hasattr(fig, 'axes'):
                logger.warning("Cannot update non-Matplotlib figure with MatplotlibBoxPlot")
                return fig
