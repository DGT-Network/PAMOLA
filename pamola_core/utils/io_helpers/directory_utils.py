...
def get_unique_filename(
    ...
    filename = get_timestamped_filename(
        base_name=base_name, extension=extension, include_timestamp=include_timestamp
    )
    ...
    if file_path.exists():
        file_path = make_unique_path(base_path=file_path)
...
def get_temp_file(
    ...
    temp_filename = f"{original_path.stem}_tmp_{file_id}{suffix}"
...
def get_temp_file_for_encryption(
    ...
    Path.
        Path to the temporary file
    """
    # Use the same logic as temporary file, but with encryption suffix
    return get_temp_file(original_file=original_file, suffix=suffix, create=create)
...
def get_temp_file_for_decryption(
    ...
    Path.
        Path to the temporary file
    """
    # Use the same logic as temporary file, but with decryption suffix
    return get_temp_file(original_file=original_file, suffix=suffix, create=create)
...
def make_unique_path(
    ...
    if not base_path.exists():
        if create and is_path_directory(base_path):
    ...
    if base_path.is_file():
        ...
        counter = 1
        while True:
            new_path = base_path.parent / f"{name_part}{separator}{counter}{extension}"
            if not new_path.exists():
                if create and is_path_directory(new_path):
    ...
    if base_path.is_dir():
        counter = 1
        while True:
            new_path = Path(f"{base_path}{separator}{counter}")
            if not new_path.exists():
                if create and is_path_directory(new_path):
                    ensure_directory(new_path)
                return new_path
...
def create_secure_directory_structure(
    ...
    for subdir in subdirs:
        ...
        safe_subdir = subdir.replace("..", "__").lstrip("/").lstrip("\\")
...
def create_secure_path(
    ...
    if create:
        if is_path_directory(secure_path):
