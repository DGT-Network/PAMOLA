...
def test_normality(
    data: pd.Series, test_method: str = "all", sample_limit: int = 5000
) -> Dict[str, Any]:
    """
...
def detect_outliers(data: pd.Series, method: str = "iqr", **params) -> Dict[str, Any]:
    """
...
def detect_outliers_iqr(data: pd.Series, iqr_factor: float = 1.5) -> Dict[str, Any]:
    """
...
def detect_outliers_modified_zscore(
    data: pd.Series, threshold: float = 3.5
) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        mad = (
            data - median
        ).abs().median() * 1.4826  # Consistency factor for normal distribution
