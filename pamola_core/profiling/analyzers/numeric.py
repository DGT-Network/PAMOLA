...
Description:
  This module provides analyzers and operations for profiling numeric fields in tabular datasets.
  It includes statistical analysis, distribution analysis, outlier detection, normality testing,
  and visualization capabilities, supporting both pandas and Dask DataFrames.

Key Features:
  - Descriptive statistics and extended metrics for numeric fields
  - Outlier detection and normality testing
  - Histogram, boxplot, and Q-Q plot visualization generation
...
class NumericAnalyzer:
    ...
    def analyze(
        ...
        **kwargs : dict
        ...
        if track_progress:
            progress_steps = (
                6 if should_test_normality else 5
            )  # Add step for normality testing
        ...
        if progress:
            progress.update(
                1, {"step": "Basic statistics", "valid_records": valid_count}
            )
        ...
        if valid_count == 0:
            ...
            try:
                if not flag_processed:
                    ...
                    stats_dict = calculate_extended_stats(
                        valid_data, near_zero_threshold
                    )
            ...
            if progress:
                progress.update(
                    1, {"step": "Descriptive statistics", "stats_calculated": True}
                )
            ...
            if semantic_analysis:
                ...
                if any(
                    term in field_lower
                    for term in ["salary", "price", "cost", "amount", "budget"]
                ):
            ...
            if should_detect_outliers:
                ...
                if (
                    should_test_normality and valid_count >= 8
                ):  # Minimum sample size for meaningful normality tests
                    try:
                        normality_results = test_normality(
                            valid_data, normality_test_method
                        )
                        ...
                        if progress:
                            progress.update(
                                1,
                                {
                                    ...
                                    "is_normal": normality_results.get(
                                        "is_normal", False
                                    ),
                                },
                            )
                        ...
                        stats_dict["normality"] = {"error": str(e), "is_normal": False}
                else:
        ...
        results = {
            ...
            "null_percentage": (
                float(round((null_count / total_rows) * 100, 2))
                if total_rows > 0
                else 0.0
            ),
            "stats": stats_dict,
...
class NumericOperation(FieldOperation):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        """
        Initialize the NumericOperation.

        Parameters
        ----------
        field_name : str
            Name of the numeric field to analyze.
        bins : int, optional
            Number of bins for histogram analysis.
        detect_outliers : bool, optional
            Whether to detect outliers in numeric data.
        test_normality : bool, optional
            Whether to perform normality tests.
        near_zero_threshold : float, optional
            Threshold below which values are treated as near-zero.
        profile_type : str, optional
            Type of profiling (default: 'numeric').
        **kwargs
        ...
        kwargs.setdefault(
            "description",
            f"Analysis of numeric field '{field_name}'",
        )
        ...
        # Inject config into kwargs for FieldOperation
        ...
        try:
            ...
            if self.use_cache and not self.force_recalculation:
                ...
                if cached_result:
                    ...
                    if progress_tracker:
                        progress_tracker.update(
                            6, {"step": "Loaded from cache", "field": self.field_name}
                        )
            ...
            if "outliers" in stats_dict:
                ...
                result.add_metric(
                    "outliers_percentage", stats_dict["outliers"].get("percentage", 0)
                )
    ...
    def _generate_visualizations(
        ...
        **kwargs_encryption,
        ...
        if "histogram" in stats_dict:
            ...
            if (
                histogram_data
                and histogram_data.get("bins")
                and histogram_data.get("counts")
            ):
                hist_filename = (
                    f"{self.field_name}_distribution_{operation_timestamp}.png"
                )
        ...
        if len(valid_data) > 5:
            boxplot_filename = f"{self.field_name}_boxplot_{operation_timestamp}.png"
        ...
        if self.test_normality and "normality" in stats_dict and len(valid_data) > 10:
            qq_filename = f"{self.field_name}_qqplot_{operation_timestamp}.png"
            qq_path = vis_dir / qq_filename

            # Generate synthetic normal data for comparison
            np.random.seed(42)  # For reproducibility
            normal_data = np.random.normal(
                loc=valid_data.mean(), scale=valid_data.std(), size=len(valid_data)
            )
            normal_data.sort()
        ...
        **kwargs_encryption,
        ...
        try:
            ...
            if viz_thread.is_alive():
                ...
                logger.info(
                    f"[DIAG] Generated visualizations: {[v.get('description', v.get('path', str(v))) for v in visualization_paths]}"
                )
        except Exception as e:
            logger.error(
            ...
            try:
                ...
                if overall_tracker:
                    overall_tracker.update(
                        0,
                        {"field": field, "progress": f"{i + 1}/{len(numeric_fields)}"},
                    )
