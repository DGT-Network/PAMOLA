...
def validate_strategy(
    strategy: str, valid_strategies: List[str], operation_type: Optional[str] = None
) -> ValidationResult:
    """
    ...
    return ValidationResult(
        is_valid=True,
        field_name="strategy",
        details={"strategy": strategy, "operation_type": operation_type},
    )


@standard_validator(field_name="strategy")
def validate_generalization_strategy(strategy: str, data_type: str) -> ValidationResult:
    """
    ...
    if data_type not in GENERALIZATION_STRATEGIES:
        return ValidationResult(
            ...
            details={
                "data_type": data_type,
                "valid_types": list(GENERALIZATION_STRATEGIES.keys()),
            },
        )
...
def validate_operation_mode(
    mode: str, valid_modes: Optional[List[str]] = None
) -> ValidationResult:
    """
    ...
    return ValidationResult(is_valid=True, field_name="mode", details={"mode": mode})


@standard_validator(field_name="null_strategy")
@validate_types(strategy=str)
def validate_null_strategy(
    strategy: str, valid_strategies: Optional[List[str]] = None
) -> ValidationResult:
    """
    ...
    return ValidationResult(
        is_valid=True, field_name="null_strategy", details={"strategy": strategy}
    )
...
def validate_bin_count(
    bin_count: int, min_bins: int = 2, max_bins: int = 1000
) -> ValidationResult:
    """
    ...
    return ValidationResult(
        ...
        details={"bin_count": bin_count, "min_bins": min_bins, "max_bins": max_bins},
    )
...
def validate_precision(
    precision: int, min_precision: int = -10, max_precision: int = 10
) -> ValidationResult:
    """
...
def validate_percentiles(percentiles: List[float]) -> ValidationResult:
    ...
    for i, p in enumerate(percentiles):
        try:
            ...
            raise InvalidParameterError(
                param_name=f"percentiles[{i}]", param_value=p, reason="Must be numeric"
            )
    ...
    return ValidationResult(
        is_valid=True,
        field_name="percentiles",
        details={"percentiles": sorted(percentiles), "count": len(percentiles)},
    )
...
def validate_noise_parameters(
    noise_level: float, strategy: str, bounds: Optional[Tuple[float, float]] = None
) -> ValidationResult:
    """
    ...
    result.details.update({"noise_level": noise_level, "strategy": strategy})

    return result


@standard_validator()
def validate_masking_parameters(
    mask_char: str, preserve_length: bool = True, pattern: Optional[str] = None
) -> ValidationResult:
    """
...
def validate_hierarchy_parameters(
    hierarchy_depth: int, merge_threshold: Optional[int] = None
) -> ValidationResult:
    """
...
def validate_strategy_compatibility(
    strategy: str, field_type: str, operation_type: str
) -> ValidationResult:
    """
...
@standard_validator()
def validate_output_field_configuration(
    df: pd.DataFrame,
    mode: str,
    ...
    if mode == "ENRICH":
        ...
        if output_field_name == original_field_name:
            result.warnings.append("Output field same as original field in ENRICH mode")
