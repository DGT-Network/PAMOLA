...
class DateTimeOpTooltip:
    ...
    interval_unit = (
        "What it does: Sets the unit of measurement for the Interval Size."
    )
    ...
    strftime_output_format = (
        "How it works: Uses standard date formatting codes (e.g., %Y-%b) to create a custom string. Note: This will convert your date column into a text column."
    )
    ...
    default_timezone = (
        "What it does: When converting to UTC, if a timestamp has no timezone info, it's assumed to be in this timezone first."
    )
    ...
    output_field_name = (
        "Name the new column. If left blank, a name is auto-generated using the prefix below (e.g., _login_time)."
    )
    
    column_prefix = (
        "Prefix for the new column name. The default is an underscore (_), which turns login_time into _login_time."
    )
    ...
    force_recalculation = (
        "Ignore saved results. Check this to force the operation to run again instead of using a cached result."
    )
