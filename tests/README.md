...
```bash
PYTHON="C:/Users/thuan.dang/AppData/Local/Programs/Python/Python312/python.exe"
...
# Run with coverage
$PYTHON -m pytest tests/ --cov=pamola_core --cov-report=html --cov-report=term-missing
...
|------|-------|--------|-------|
| **Tier 1** | Public API (39 ops + 17 functions) | >=90% coverage | 2,011 |
| **Tier 2** | Framework core (BaseOperation, BaseTask, privacy_models, attacks, cli) | >=85% coverage | 1,516 |
...
### Tier 1 — Public API (61 files, 2,011 tests)

| Directory | Files | Tests | What it tests |
|-----------|-------|-------|---------------|
| `analysis/` | 5 | 219 | `analyze_dataset_summary`, `calculate_full_risk`, `analyze_descriptive_stats`, `visualize_distribution_df`, `analyze_correlation` |
| `anonymization/` | 11 | 322 | 10 anonymization ops + `BaseAnonymizationOperation` |
| `profiling/` | 13 | 434 | 14 profiling analyzers (anonymity, attribute, categorical, correlation, currency, date, email, group, identity, mvf, numeric, phone, text) |
| `transformations/` | 9 | 363 | 8 transformation ops + `BaseTransformationOperation` |
| `metrics/` | 11 | 240 | 3 metric ops + `BaseMetricsOp` + individual metrics (KS, KL, DCR, NNDR, F1, R2, MSE) |
...
### Tier 2 — Framework Core (66 files, 1,516 tests)

| Directory | Files | Tests | What it tests |
|-----------|-------|-------|---------------|
| `utils/ops/` | 8 | ~200 | `BaseOperation`, `OperationConfig`, `DataSource`, `DataWriter`, `OperationResult`, `OperationRegistry`, `OperationCache` |
| `utils/tasks/` | 14 | ~300 | `BaseTask`, `TaskConfig`, `DependencyManager`, `EncryptionManager`, `ProgressManager`, `PathSecurity`, `ProjectConfigLoader`, `TaskRegistry`, `OperationExecutor` |
| `utils/nlp/` | 19 | ~200 | NLP base, cache (memory/file/model), tokenization (spacy/transformers), entity extraction |
| `utils/test_io.py` | 1 | 16 | I/O helper functions |
| `privacy_models/` | 5 | 173 | `KAnonymityProcessor`, `LDiversityCalculator`, `TCloseness`, `DifferentialPrivacyProcessor` |
| `attacks/` | 7 | 181 | `LinkageAttack`, `MembershipInference`, `AttributeInference`, `DistanceToClosestRecord`, `NearestNeighborDistanceRatio`, `AttackMetrics` |
