...
class TestEmailOperation(unittest.TestCase):
    ...
    def test_assess_privacy_risk_total_valid_zero(self):
        df = pd.DataFrame({'email': [None]})
        op = EmailOperation(field_name='email')

        result = op._assess_privacy_risk(df, 'email')
...
class TestAnalyzeEmailFields(unittest.TestCase):
    ...
    @patch('pamola_core.profiling.analyzers.email.load_data_operation')
    def test_default_email_field_when_none_detected(self, mock_load_data):
        # DataFrame with no column containing 'email'
        df = pd.DataFrame({'username': ['a', 'b'], 'contact': ['x', 'y']})
        mock_load_data.return_value = df

        # Call without email_fields argument
        results = analyze_email_fields(
            data_source=self.data_source,
            task_dir=self.task_dir,
            reporter=self.reporter
            # email_fields is omitted
        )

        # Check that the default ['email'] was used
        self.reporter.add_operation.assert_any_call(
            "Email fields analysis",
            details={
                "fields_count": 1,
    ...
    def test_progress_tracker_update_on_error(self, mock_load_data, mock_progress, mock_email_op):
        ...
        # Mock EmailOperation.execute returns error
        error_result = OperationResult(status=OperationStatus.ERROR, error_message="Some error")
        instance = mock_email_op.return_value
        instance.execute.return_value = error_result
        ...
        # Ensure update is called with status error and error_message
        mock_tracker.update.assert_any_call(1, {
            "field": "email",
            "status": "error",
            "error": "Some error"
        })
    ...
    def test_execute_raises_exception_triggers_error_handling(
        self, mock_load_data, mock_logger, mock_progress, mock_email_op
    ):
        # Setup DataFrame with one email field
        df = pd.DataFrame({'email': ['a@a.com', 'b@b.com']})
        mock_load_data.return_value = df
        ...
        # Mock EmailOperation instance, make execute raise exception
        instance = mock_email_op.return_value
        instance.execute.side_effect = Exception("Test exception")
        ...
        self.reporter.add_operation.assert_any_call(
            "Analyzing email field", status="error", details={"error": "Test exception"}
        )
        # Check overall_tracker.update was called with status="error"
