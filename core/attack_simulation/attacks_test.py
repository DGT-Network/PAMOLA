...
import numpy as np
import pandas as pd
from pamola.core.attack_simulation.attack_metrics import AttackMetrics
from pamola.core.attack_simulation.attribute_inference import AttributeInference
from pamola.core.attack_simulation.membership_inference import MembershipInference
from pamola.core.attack_simulation.linkage_attack import LinkageAttack
...
class AttacksTest:
    ...
    def __init__(self, attribute_inference, membership_inference, linkage_attack, attack_metrics):
    ...
    def generate_name_abbreviations(self, full_names):
        ...
        for name in full_names:
            ...
            if len(words) > 1:
                abbreviation = " ".join([w[0] + "." for w in words[:-1]]) + " " + words[-1]  # Abbreviate the first words, keep the last word the same
            else:
    ...
    def generate_city_abbreviations(self, names):
        ...
        for name in names:
            words = name.replace(",", "").split()  # Separate words, remove commas if any
            if len(words) > 1:
                abbreviation = "".join(word[0] for word in words).upper()  # Choose the first letter of each word
            else:
                abbreviation = name[:3].upper()  # If there is only one word, take the first three letters
            abbreviations.append(abbreviation)
    ...
    def generate_data(self, file_path = None):
        ...
        if file_path is None:
            ...
            # List of first_names
            first_names = [
                "John", "Alice", "David", "Emma", "Michael", "Sophia", "James", "Olivia", "Daniel", "Emily", "Robert",
                "Isabella", "William", "Mia", "Joseph", "Charlotte", "Charles", "Amelia", "Thomas", "Harper", "Henry",
                "Evelyn", "Christopher", "Abigail", "Matthew", "Ella", "Joshua", "Scarlett", "Andrew", "Madison",
                "Nathan", "Avery", "Samuel", "Lily", "Benjamin", "Grace", "Jack", "Chloe", "Ryan", "Zoe", "Jacob",
                "Victoria", "Logan", "Penelope", "Lucas", "Riley", "Mason", "Layla", "Ethan", "Nora"
            ]
            ...
            last_names = [
                ...
                "Lee", "Perez", "Thompson", "White", "Harris", "Clark", "Lewis", "Walker", "Hall", "Allen", "Young",
                "King", "Wright", "Scott", "Green", "Adams", "Baker", "Nelson", "Carter", "Mitchell", "Perez", "Roberts",
                "Phillips", "Evans", "Campbell", "Edwards", "Collins", "Stewart", "Sanchez", "Morris"
            ]
            ...
            city = [
                ...
                "Indianapolis", "Seattle", "Denver", "Washington, D.C.", "Boston", "El Paso", "Nashville", "Detroit",
            ...
            data = pd.DataFrame({
                "ID_DF": range(1, 101),
                "FULLNAME": [f"{np.random.choice(first_names)} {np.random.choice(last_names)}" for _ in range(100)],
                "AGE": np.random.randint(20, 60, 100),  # Random age from 20 to 60
                "SEX": np.random.choice(["Male", "Female"], 100),  # Random SEX
                "RACE": np.random.choice(["Asian", "White", "Black", "Latino", "Others"], 100),
                "CITY": np.random.choice(city, 100),
                "Education": np.random.choice(["High School", "Bachelor", "Master"], 100),  # Random education level
                "Income": np.random.randint(30000, 100000, 100)  # Random income from 30,000 to 100,000
            })
        else:
        ...
        for col in data.columns:
            if data[col].dtype == "object":  # If the column is string replace None values with empty
                data[col] = data[col].replace({None: ""})
            elif np.issubdtype(data[col].dtype,
                               np.number):  # If the column is numeric, replace None or nan values with 0
    ...
    def create_data_test_mia(self, data):
        ...
        for col in data_test.columns:
            if data_test[col].dtype == "object":  # If the column is string replace None values with empty
                data_test[col] = data_test[col].replace({None: ""})
            elif np.issubdtype(data_test[col].dtype, np.number):  # If the column is numeric, replace None or nan values with 0
    ...
    def create_data_test_linkage_attack(self, data = None):
        ...
        if data is None:
            ...
            df1 = pd.DataFrame({
                "ID_DF1": [1, 2, 3, 4, 5],
                "FULLNAME": ["John Doe", "Alice Smith", "Bob Brown", "Charlie Johnson", "David Lee"],
                "AGE": [25, 30, 35, 40, 45],
            ...
            df2 = pd.DataFrame({
                "ID_DF2": ["A", "B", "C", "D", "E"],
                "FULLNAME": ["J. Doe", "A. Smith", "B. Brown", "C. Johnson", "D. Lee"],  # Acronym
    ...
    def display_membership_inference_attack_results(self, actual_values, prediction_values):
        """
        ...
        # Attack Assessment
        attack_metrics_values = self.attack_metrics.attack_metrics(actual_values, prediction_values)
    ...
    def save_membership_inference_attack_results(self, actual_values, prediction_values, file_path):
        """
    ...
    def display_attribute_inference_attack_results(self, actual_values, prediction_values):
        """
    ...
    def save_attribute_inference_attack_results(self, actual_values, prediction_values, file_path):
        """
    ...
    def save_linkage_attack_results(self, results, file_path, title):
        """
        Save the result of Linkage Attack to file
        """
        ...
        with open(file_path, "w", encoding="utf-8") as file:
            ...
            for row in results.itertuples(index=False):
                score = f"{row.Score:.6f}" if hasattr(row, "Score") else ""
                line = f"{row.ID_DF1:^7} | {row.ID_DF2:^7} | {score}\n"
                print(line, end="")
                file.write(line)
    ...
    def run_test_membership_inference_attack_dcr(self, data_train, data_test):
        ...
        self.save_membership_inference_attack_results(actual_values, prediction_values, "pamola/core/attack_simulation/results/mia_dcr.txt")
    ...
    def run_test_membership_inference_attack_nndr(self, data_train, data_test):
        ...
        self.save_membership_inference_attack_results(actual_values, prediction_values, "pamola/core/attack_simulation/results/mia_nndr.txt")
    ...
    def run_test_membership_inference_attack_model(self, data_train, data_test):
        ...
        self.save_membership_inference_attack_results(actual_values, prediction_values, "pamola/core/attack_simulation/results/mia_model.txt")
    ...
    def run_test_attribute_inference_attack(self, data_train, data_test):
        ...
        self.save_attribute_inference_attack_results(actual_values, prediction_values, "pamola/core/attack_simulation/results/attribute_inference_attack.txt")
    ...
    def run_test_record_linkage_attack(self, data1, data2):
        ...
        self.save_linkage_attack_results(results, "pamola/core/attack_simulation/results/record_linkage_attack.txt", "Record Linkage Attack results")
    ...
    def run_test_probabilistic_linkage_attack(self, data1, data2):
        ...
        self.save_linkage_attack_results(results, "pamola/core/attack_simulation/results/probabilistic_linkage_attack.txt", "Probabilistic Linkage Attack results")
    ...
    def run_test_cluster_vector_linkage_attack(self, data1, data2):
        ...
        self.save_linkage_attack_results(results, "pamola/core/attack_simulation/results/cluster_vector_linkage_attack.txt", "Cluster Vector Linkage Attack results")


    def run_test(self):
        data = self.generate_data()
        #data = self.generate_data("pamola/core/attack_simulation/data_files/RandomPeople_5.csv")
        ...
        print("\n🔹 How to run run_test() function?")
        print("\n🔹 Terminal -> cd pamola_core -> enter: py -m pamola.core.attack_simulation.attacks_test\n")


### Initialize objects
attribute_inference = AttributeInference()
membership_inference = MembershipInference(dcr_threshold = None, nndr_threshold = None, m_threshold = None)
linkage_attack = LinkageAttack(fs_threshold = None, n_components = 4)
attack_metrics = AttackMetrics()

### Initialize AttacksTest object
attacks_test = AttacksTest(attribute_inference, membership_inference, linkage_attack, attack_metrics)
