...
>
> These datasets are **fully synthetic** - artificially generated for testing purposes.
> **No real personal information (PII) is included.**
>
> - Not collected from any external sources
> - Not derived from any real customer or financial institution data
> - Any resemblance to actual persons or transactions is coincidental
>
...
| Component | Approach |
|-----------|----------|
| Identifiers | UUID-based with format prefixes |
| Demographics | Faker library with Canadian locale |
| Financial values | Statistical distributions based on public aggregate data |
| Correlations | Rule-based logic (e.g., income vs. credit limit) |
| Linkage IDs | SHA-256 hashes of synthetic seed values |

**Generated:** 2024 for PAMOLA Epic 2 testing

**Reproducibility:** Seed-based generation documented in passport JSON files.

---

## Datasets

### 1. CREDIT_BANK_APPS - Bank Loan Applications Dataset

| Property | Value |
|----------|-------|
| **File** | `CREDIT_BANK_APPS_10k.csv` |
...
| **Dictionary** | `CREDIT_BANK_APPS_dictionary.csv` |
| **Passport** | `CREDIT_BANK_APPS_passport.json` |

### 2. CREDIT_BUREAU - Credit Bureau Dataset

| Property | Value |
|----------|-------|
| **File** | `CREDIT_BUREAU_10k.csv` |
...
| **Dictionary** | `CREDIT_BUREAU_dictionary.csv` |
| **Passport** | `CREDIT_BUREAU_passport.json` |

---

## Field Descriptions

### CREDIT_BANK_APPS Fields (38 fields)
...
| Field | Description |
|-------|-------------|
| `gt_person_id` | Ground-truth person ID for evaluation only - **remove before release** |
...
### CREDIT_BUREAU Fields (30 fields)
...
| **Differential Privacy** | Apply Laplace noise (epsilon ~0.2) to numeric fields |
...
|---------|----------------|
| Income Verification | Verified income ~ 0.95 x declared income (+/- noise) |
...
- Add Laplace noise to financial amounts (epsilon ~0.2)
...
```
bank_credit_ca/
├── CREDIT_BANK_APPS_10k.csv         # Bank applications dataset
├── CREDIT_BANK_APPS_dictionary.csv  # Field definitions
├── CREDIT_BANK_APPS_passport.json   # Generation specifications
├── CREDIT_BUREAU_10k.csv            # Credit bureau dataset
├── CREDIT_BUREAU_dictionary.csv     # Field definitions
├── CREDIT_BUREAU_passport.json      # Generation specifications
...
> **Reminder:** This is synthetic test data with no real PII.

| Note | Details |
|------|---------|
| **Evaluation fields** | `gt_*` prefix fields are ground-truth for testing only - remove before any external use |
| **Linkage fields** | `link_shared_id` enables cross-dataset joins for federated learning tests |
| **Compliance testing** | Suitable for testing GDPR/CCPA/fair lending workflows (not a compliance certification) |
| **Passport files** | Refer to JSON passports for complete field specifications and generation parameters |
